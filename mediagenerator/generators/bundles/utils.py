from .settings import ROOT_MEDIA_FILTERS, MEDIA_BUNDLES, BASE_ROOT_MEDIA_FILTERS, POST_ROOT_MEDIA_FILTERS, MEDIA_BUNDLES
from mediagenerator.settings import MEDIA_DEV_MODE, ENABLE_LESS_CSS_DEBUG
from mediagenerator.utils import load_backend, media_urls
import os

_cache = {}

_cached_media_bundles = None

def _get_media_bundles():
    global _cached_media_bundles
    if _cached_media_bundles is None:
        _cached_media_bundles = _process_includes(MEDIA_BUNDLES)
    return _cached_media_bundles
...
def _get_root_filters_list(filetype):
    root_filters = ()
    filetypes = (filetype, '*')
    for filters_spec in (BASE_ROOT_MEDIA_FILTERS, ROOT_MEDIA_FILTERS, POST_ROOT_MEDIA_FILTERS):
