...
def get_url_var(key):
    ...
    a = tail.split('.')[0][:3]
    return int(sum([ord(c) for c in a])/10)

def rewrite_url(key):
    media_change = getattr(settings, 'ENABLE_MULTI_DOMAIN_MEDIA', False)
    if (media_change):
        url_pattern = settings.MEDIA_URL_PATTERN
        a = get_url_var(key)
        return url_pattern.replace('%d', str(a))
    else:
        return ''
...
def media_urls(key, refresh=False):
    if media_settings.MEDIA_DEV_MODE:
        if refresh:
            _refresh_dev_names()
        return [rewrite_url(key) + DEV_MEDIA_URL + url for url in _generated_names[key]]
    return [rewrite_url(key) + PRODUCTION_MEDIA_URL + get_production_mapping()[key]]


def media_url(key, refresh=False):
    urls = media_urls(key, refresh=refresh)
    if len(urls) == 1 or len(set(urls)) == 1:
        return urls[0]
